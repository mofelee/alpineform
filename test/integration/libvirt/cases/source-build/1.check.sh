...
assert_remote "temporary build ownership is clean and shared world intent remains" \
  "! grep -Eq '^\\.alpineform-build-' /etc/apk/world && ! apk info | grep -Eq '^\\.alpineform-build-' && grep -qx zlib-dev /etc/apk/world && apk info -e zlib-dev && ! apk info -e build-base && ! apk info -e bubblewrap"
