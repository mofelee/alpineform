...
func parseMovedComponentTraversal(file, endpoint string, expr hcl.Expression) (string, error) {
	...
	if diags.HasErrors() || len(traversal) != 2 {
		return "", movedTraversalError(file, endpoint, expr)
	}
	root, rootOK := traversal[0].(hcl.TraverseRoot)
	name, nameOK := traversal[1].(hcl.TraverseAttr)
	if !rootOK || root.Name != "component" || !nameOK || !declarationLabelPattern.MatchString(name.Name) {
		return "", movedTraversalError(file, endpoint, expr)
	}
	return name.Name, nil
}

func movedTraversalError(file, endpoint string, expr hcl.Expression) error {
...
func validateAndSortMoved(cfg *Config) error {
	...
	for _, move := range moves {
		...
		if previous, ok := byTarget[move.To]; ok && previous.From != move.From {
			return movedError(move.ToSource, "component.%s and component.%s both map to component.%s; first defined at %s:%d", previous.From, move.From, move.To, previous.Source.File, previous.Source.Line)
		}
		bySource[move.From] = move
		byTarget[move.To] = move
	}

	state := make(map[string]uint8, len(bySource))
