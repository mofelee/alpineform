...
func ParseVariableFile(path string) ([]ExternalVariableValue, error) {
	if !strings.HasSuffix(path, product.VarFileSuffix) && !strings.HasSuffix(path, product.VarJSONSuffix) {
		return nil, fmt.Errorf("variable file %s must use the *.apfvars or *.apfvars.json suffix", path)
	}
